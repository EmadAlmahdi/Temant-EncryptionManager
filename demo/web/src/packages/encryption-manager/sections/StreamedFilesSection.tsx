...
import { runStreamedRoundTrip, type StreamReport } from "../../../lib/api";
import { formatBytes } from "../../../lib/format";

const SAMPLE_SIZE_BYTES = 256 * 1024;
...
export function StreamedFilesSection() {
  ...
  const pickFile = (picked: File | null) => {
    ...
    if (picked && picked.size > MAX_UPLOAD_BYTES) {
      setSizeError(`That file is ${formatBytes(picked.size)}; this demo caps uploads at ${formatBytes(MAX_UPLOAD_BYTES)}.`);
      setFile(null);
      return;
    }
    setSizeError(null);
  ...
  const run = async () => {
    ...
    setLoading(true);
    setReport(null);
    const res = await runStreamedRoundTrip(file, password, chunkSize);
    setReport(res);
    setLoading(false);
  };

  return (
    <TerminalWindow label="stream.sh" meta="Chunked AEAD">
      <div className="mb-5 flex items-center gap-2.5">
        <FileLock2 className="h-4 w-4 text-amber-500" />
        <span className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">Streamed file encryption</span>
      </div>

      <p className="mb-4 text-sm leading-relaxed text-zinc-500 dark:text-zinc-400">
        Encrypts and decrypts a file in fixed-size chunks instead of loading it fully into memory,
        then confirms the round trip is byte-for-byte identical and that a single flipped byte in
        the ciphertext is detected rather than silently accepted.
      </p>
          ...
          <div className="mt-1 animate-rise overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950 px-3.5 py-3 font-mono text-xs">
            {report.ok ? (
              <div className="space-y-1">
                ...
                <StatLine label="encrypted" value={formatBytes(report.encryptedBytes ?? 0)} tone="ok" />
                <StatLine label="chunks" value={`${report.chunkCount} × ${formatBytes(report.chunkSize ?? 0)}`} tone="ok" />
                <StatLine
